...
const RecipeFilterApp = () => {
  ...
  const averageRating = filterRecipe.reduce((sum, r) =>
    sum = sum + r.rating

    , 0) / (filterRecipe.length || 1);
  ...
  return (
    <div className="receipe-header">
      ...
      <div className="filter-cart">
        <label htmlFor="ratingFilter">Filter by Rating: </label><select value={minRating} id="ratingFilter"
      ...
      <h1 className="rating">Average Rating: {averageRating.toFixed(2)} ({filterRecipe.length} recipes)</h1>
      ...
      <div className="food-cart">
        {filterRecipe.map((recipe) => (
          <div className="menu" key={recipe.id}>
            ...
            <button onClick={() => updateCart(recipe)} className="cart-btn">Add to Cart</button>
          </div>
        ))}
      </div>
    </div>
