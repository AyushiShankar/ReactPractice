          ...
          <div key={col} className="column" data-testid={`column-${col}`}>
            ...
            {tasks.filter((task) => task.status === col).map((task) => (
              <div key={task.id} className="task-card" data-testid={`task-${task.id}`}>
                {editingId === task.id ? (
                  <input autoFocus data-testid={`task-title-edit-${task.id}`}
                    value={editingTitle}
                ...
                <div className="task-buttons">
                  {col !== "todo" && (<button data-testid={`move-left-${task.id}`}
                    onClick={() => moveTask(task.id, "left")}
                    className="shiftBtn"
                  >L</button>
                  )}
                  {col !== "completed" && (<button data-testid={`move-right-${task.id}`}
                    onClick={() => moveTask(task.id, "right")}
                    className="shiftBtn"
                  >R</button>
                  )}
                  <button data-testid={`edit-button-${task.id}`}
                    onClick={() => {
                      setEditingId(task.id);
                      setEditingTitle(task.title);
                    }}
                    className="editBtn">Edit</button>
                  <button data-testid={`delete-button-${task.id}`}
                    onClick={() => {
                      deleteTask(task.id)
                    }}
