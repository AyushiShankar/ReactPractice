...
export default function BillingCounter() {
  ...
  const addCounters = () => {
    ...
    const newCounters = Array.from({ length: Number(counterNo) }, (_, index) => ({
      id: `${index + 1}`,
  ...
  return (
    <div className="billing-container" data-testid="billing-container">
      ...
      <div className="input-section" data-testid="customer-input-section">
        ...
        />
          <button data-testid="set-customer-button"
            onClick={addCustomer}>
            Add Customer
          </button></>) : (<>
            <input
            ...
            />
            <button data-testid="set-counter-button"
              onClick={addCounters}>
              Set Counters
            </button></>)}
      </div>
      {message && <p className="assigned-msg" data-testid="assignment-msg">{message}
      </p>}
      {counters.length > 0 &&
        <div className="counter-wrapper">{counters.map((counter, index) => (
