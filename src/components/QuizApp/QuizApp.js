...
export default function QuizApp({ questions }) {
  ...
  const handleSelection = (e) => {
    ...
    if (currentIndex < totalQuestion) {
      try {
        if (!selectedAns) return setMessage("Please select an option before submitting.");
  ...
  return (
    <div className="App">
      ...
      {(totalQuestion === 0) && !questions && <p className="warning" data-testid="warning">No quiz available</p>}
      {(totalQuestion > 0 && !(currentIndex >= totalQuestion)) && <form className="question-container">
        ...
        {message && <p className="warning" data-testid="warning">{message}</p>}
        <button type="submit" className="submit-button"
          onClick={(e) => handleSelection(e)}>
          Submit
        </button>
      </form>}
      {(currentIndex >= totalQuestion) && totalQuestion > 0 && <div className="score-container">
